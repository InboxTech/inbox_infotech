...
class ProductSliderController extends Controller
{
    public function index()
    {
        ...
        return view('admin/product_banner',$result);
    }

    
    public function manage_product_banner(Request $request,$id='')
    {
        if($id>0){
        ...
        return view('admin/manage_product_banner',$result);
    }

    public function manage_product_banner_process(Request $request)
    {
        if($request->post('id') == '')
        {
        $request->validate([
        ...
        if($request->post('id')>0){
            ...
            $msg="Product Banner updated";
        }else{
            $model=new Slider();
            $msg="Product Banner inserted";
        }
        ...
        return redirect('admin/product_banner');
    ...
    public function delete(Request $request,$id){
        ...
        $model->delete();
        $request->session()->flash('message','Product Banner deleted');
        return redirect('admin/product_banner');
    }
    ...
    public function status(Request $request,$status,$id){
        ...
        $model->status=$status;
        $model->save();
        $request->session()->flash('message','Product Banner status updated');
        return redirect('admin/product_banner');
    }
}
