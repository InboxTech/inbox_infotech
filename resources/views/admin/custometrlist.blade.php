...
<div class="col-md-12">
<!-- DATA TABLE -->
<h3 class="title-5 m-b-35">Customers List</h3>
<div class="table-data__tool">
    <form method="post" action="{{url('/admin/customers/list/search/')}}">
    {{ csrf_field() }}
...
<a href="{{url('/admin/customers/add')}}">
...
<tbody>
    ...
    <tr class="tr-shadow">
        <td>{{$a++}}</td>
        <td>
            <span><?php echo $msgTrimmed = mb_substr($list->firstname,0,40);?></span>
        </td>
		<td>
		...
		<td><span class="block-email">{{$list->name}}</span></td>
        <td>
            @if($list->status == '1')
            Active
            @else
        ...
        <td>
            <div class="table-data-feature">
                <a href="{{url('/admin/customers/update/')}}/{{$list->id}}">
                <button class="item" data-toggle="tooltip" data-placement="top" title="Edit">
                    <i class="zmdi zmdi-edit"></i>
                </button>
                </a>
                <a  onclick="return myFunction();" href="{{url('/admin/customers/delete/')}}/{{$list->id}}">
